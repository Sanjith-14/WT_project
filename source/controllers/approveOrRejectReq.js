...
approveRequest = async (req, res) => {
    ...
    const detail = await Detail.updateMany(
        { "mails.toMail": email },
        { $set: { "mails.$[ele].approvedFlag": true, "mails.$[ele].approverContent": content, "mails.$[ele].approveOrRejectTime": new Date(), "mails.$[ele].pendingFlag": false } },
...
rejectRequest = async (req, res) => {
    ...
    const detail = await Detail.updateMany(
        { "mails.toMail": email },
        { $set: { "mails.$[ele].rejectedFlag": true, "mails.$[ele].approverContent": content, "mails.$[ele].approveOrRejectTime": new Date(), "mails.$[ele].pendingFlag": false } },
        { arrayFilters: [{ "ele.toMail": email, "ele._id": id }] }
    )
